...
#define MAXSIZE 12//定义Romberg函数中二维数组的行数，该二维数组列数设为4
#define eps 0.5e-5//题中要求的误差在Romberg函数中使用
...
double T(double a, double b, int n,int z)//区间逐次分半求积法中的递推公式，写的是一个递归形式
{
	if (n == 1)//递归出口
		return  (b - a) * (f(a,z) + f(b,z)) / 2;
	else
	{//这里的想法还是跟前面的类似，都是想将完整的公式拆解
		double sum = T(a, b, n / 2,z) / 2;//公式的前半部分
		double temp = 0;
